...
class threadpool {
 ...
 public:
  ...
  template <class T, class... Args>
  void push(T &&t, Args... args) {
    function<void()> f = std::bind(std::forward<T> & t, std::forward<T> & args);
    {
      lock_guard<mutex> lock(mtx);
      threads.emplace_back(task{next_id++, f});
    }
    conidtion.notify_one();
  }
};
int main() {
  threadpool tp;
  for (int i = 0; i < 100; i++) {
    tp.push([i] { cout << i << endl; });
  }
